...
$$
which matches the familiar Euclidean [[1.4 Vector spaces#Theorem 1.3 The distance formula|distance formulas]] in $\mathbb{R}^2$ and $\mathbb{R}^3$ 
## 3.6 Orthogonality
...
\begin{flalign}
	...
	& \Rightarrow ||\vec{v}||^2 + ||\vec{w}||^2 + 0 + 0 \\
	& = ||\vec{v}||^2 + ||\vec{w}||^2
\end{flalign}
$$
idk why it refuses to render
...
The thing inside the $\cos^{-1}$ stays within the domain of the function
## 3.7 Angles
...
Then $\vec{w}_1, \vec{w}_2, \cdots, \vec{w}_m$ form an orthogonal basis for the subspace of $\mathbb{R}^n$ [[2.2 Linear dependency, spanning, and basis#2.4 Spanning|spanned]] by $\vec{v}_1, \vec{v}_2, \cdots, \vec{v}_m$
...
### Corollary 3.1
Every subspace of $\mathbb{R}^n$ has an [[#3.6 Orthogonality|orthogonal]] basis
...
Let $W$ be a [[2.1 Subspaces#2.1 Subspaces|subspace]] of $\mathbb{R}^n$. The orthogonal complement of $W$ relative to an inner product ⟨, ⟩, denoted by $W^\perp$ , is defined as
$$
