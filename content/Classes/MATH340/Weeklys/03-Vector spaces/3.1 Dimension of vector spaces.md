...
## Theorem 3.1   
Assume $V$ is a [[2.1 Subspaces#2.1 Subspaces|subspace]] of $\mathbb{R}^n$. Then, there is an integer $m ≤ n$ for which $V$ has a [[2.2 Linear dependency, spanning, and basis#2.5 Basis|basis]] consisting of $m$ distinct vectors. Furthermore, every basis of $V$ contains precisely $m$ vectors
...
(i) if **dim**$V = 0$ then $\emptyset$ is a basis of $V$ meaning $V$ is **span**$\emptyset$ i.e. $V = {0}$ (here the span of the empty set is said to be 0)
 
(ii) if **dim**$V = 1$ then $V = \text{span\{a\}}$ where $a,b \neq 0$  which is a line through the origin.

(iii) if **dim**$V = 2$ this means that $V = \text{span}\{ \vec{a} ,\vec{b} \}$ where $\vec{a},\vec{b}$ are linearly independent. We know 
$$
\textbf{dim}\mathbb{R}^2 = 2
$$
which implies that $\vec{a},\vec{b}$ linearly independent vectors form can be a basis for $\mathbb{R}^2$ meaning $\text{span}\{\vec{a},\vec{b}\} = \mathbb{R}^2 = V$. $V$ is the span of these two vectors btw.  
