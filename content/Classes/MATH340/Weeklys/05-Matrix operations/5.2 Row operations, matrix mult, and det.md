![video](https://www.youtube.com/watch?v=aP17CwRMVac&ab_channel=MathCoursesbyDr.Ebrahimian)
...
[[5.1 Determinants#5.2 Determinant|Determinants]] can be evaluated using co-factor expansion. Here is an example

![[Pasted image 20240924203454.png]]

This means that we can re-write the determinant of a $3 \times 3$ matrix $A$ as follows:
...
A square matrix $A$ is called invertible or nonsingular if there is a square matrix $B$ for which $AB = BA = I$.  It is denoted as $A^{-1}$
...
3. Rows of $A$ are [[2.2 Linear dependency, spanning, and basis#2.3 Linear dependency|linearly independent]]

4. Columns of $A$ are [[2.2 Linear dependency, spanning, and basis#2.3 Linear dependency|linearly independent]]
