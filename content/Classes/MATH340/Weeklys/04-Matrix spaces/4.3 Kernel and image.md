...
Basically the same as previous definitions of [[3.2 Pivot analysis#Null space (kernel)|kernel]] 

$L$ is a [[4.2 Linear transformations and matrix operations#4.2 Linear transformations|linear function]], so image makes sense. It also has a matrix associated with it, which is why kernel is involved.
## Theorem 4.6 
Let $L : V → W$ be a [[4.2 Linear transformations and matrix operations#4.2 Linear transformations|linear transformation]] of [[1.4 Vector spaces|vector spaces]]. Then Ker $L$ is a [[2.1 Subspaces#2.1 Subspaces|subspace]] of $V$ and Im $L$ is a subspace of $W$.
...
## Theorem 4.7
Suppose $L : V → W$ is a linear transformation of vector spaces. Then, Ker $L = {\bf 0}$ if and only if $L$ is [[1.2 Functions#Injectivity|one-to-one]]. If $L$ is one-to-one, then dim Im $L$ = dim $V$ i.e. the [[1.2 Functions#1.8 Images and pre-images|image]] is the entire domain.
## Theorem 4.8
...
Also, if a linear function is [[1.2 Functions#1.7 Functions|bijective]] then its image is equal to its codomain