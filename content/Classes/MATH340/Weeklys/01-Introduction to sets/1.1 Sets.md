...
$$
Note how the only difference between unions and intersections if the condition 'or' a/ 'and'. Interesting connection between unions and intersections,
...
By the definition of a union ([[#1.1 Unions & Intersections]]), then $(x \in A \vee x \in B) \wedge x \in C$, both statements being true
...
$$
THUS, FINALLY, it can be said that the two are equal by the definition of set equality (see [[#1.2 Equality between sets]])
$$
...
$$
\left( \bigcap\limits_{j=1}^{n} A_{j} \right)^c = \bigcup\limits_{j=1}^{n} A_{j}^c \space\space \text{ or } \space\space (A \cap B)^c = A^c \cup B^c
$$
$$
\left( \bigcup\limits_{j=1}^{n} A_{j}\right)^c = \bigcap\limits_{j=1}^{n} A_{j}^c \space\space \text{ or } \space\space (A \cup B)^c = A^c \cap B^c
$$
### Proof of first law
Show that $(A \cap B)^c = A^c \cup B^c$ by showing that $x \in (A \cap B)^c \iff x \in  A^c \cup B^c$

Let $x \in (A \cap B)^c$ , then by the definition of a [[#1.6 Difference of sets |complement]], $x \notin A \cap B$,
