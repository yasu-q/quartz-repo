Recall that if a point is a local extremum (see [[8.2 Derivatives and differentials#8.8 Critical points]]), then it must be a critical point. Let's first look at a simple case when
$$
