...
$$
tsumari, the value of $f(a + h) - f(a)$ is very close to $f'(a)h$, when $h$ is small. Note that $f'(a)h$ is a linear function
## 7.5 Derivatives 
...
## Theorem 7.7 
Suppose $f = (f_1, \cdots, f_n) : I \rightarrow \mathbb{R}^n$ is a continuous function, where $I \subseteq \mathbb{R}^n$ is an open interval. Then, $f$ is differentiable at a point $a \in I$ iff $f_j$ is differentiable for all $j, j = 1, \dots, n$ . Furthermore, if $f$ is differentiable at $a$, them $f'(a) = (f_1'(a), \dots, f_n'(a))$. Proof from [[6.1 Limits#Theorem 6.2]]
...
$$
Furthermore, when such a [[4.2 Linear transformations and matrix operations#4.2 Linear transformations|linear transformation]] exists, it is unique and $$L(h) = f'(a)h$$
### Composition of differentials
Note that 
$$
d(f \circ \phi)_a = df_{a} \circ d \phi_a
$$
i.e. the differential of $f \circ \varphi$ is the same thing as the compositions of $f$' and $\varphi$'s differentials
...
- A curve $f : I \rightarrow \mathbb{R}^n$ is called simple if it is one-to-one except possible at its endpoints. In other words, if $f(s) = f(t)$ for some $s, t \in I$ then $s = t$ or both are endpoints. See [[1.2 Functions#Injectivity]]. i.e. they are injective except, possibly, at the endpoints. THEY DON'T INTERSECT
## Theorem 7.11
Suppose $f : [a, b] \rightarrow \mathbb{R}^n$ is a simple $C^1$ curve. The arc length of this curve is given by
$$
\int_{a}^{b} ||\, f'(t) \,|| dt 
$$
(subtle foreshadowing line integrals)
***
## Example 7.10
