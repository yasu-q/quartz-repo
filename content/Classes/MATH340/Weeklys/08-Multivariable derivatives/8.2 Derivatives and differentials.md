...
We know from the definition of directional derivatives ([[8.1 Directional derivatives#8.1 Directional derivatives]]) that
$$
...
Suppose $U$ is an open subset ([[7.1 Topology of Rn#7.1 Open sets]]) of $\mathbb{R}^n$. Let $F : U → \mathbb{R}^m$ be a function that is differentiable at some $a ∈ U$ . Then $dFa : \mathbb{R}^n → \mathbb{R}^m$ is linear and its matrix $F ′({\bf a})$ is an $m × n$ matrix for which $dF_a({\bf h}) = F'({\bf a}){\bf h}$, where ${\bf h}$ is a column vector in $\mathbb{R}^n$ 

See [[4.2 Linear transformations and matrix operations#Theorem 4.3]]
## Theorem 8.4
...
### Corollary
Suppose $U$ is an open subset of $\mathbb{R}^n$ and $f : U → R$ is a function that is differentiable at some $a ∈ U$ . Then, for every ${\bf 0} \neq {\bf v} ∈ \mathbb{R}^n$ we have
$$
...
- Maximum direction derivative at ${\bf a}$ if in the direction of $\nabla f({\bf a})$
	- This maximum value is equal to $||\nabla f({\bf a})||$ 
- The minimum is just in the negative direction, equal to negative magnitude
