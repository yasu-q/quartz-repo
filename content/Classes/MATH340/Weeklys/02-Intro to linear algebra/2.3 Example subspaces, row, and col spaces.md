...
2 Closure:
Let $\vec{x}_1,\vec{y}_1\in \{span \text{ } \mathcal{A}\}$, then by definition of a [[2.2 Linear dependency, spanning, and basis#2.2 Linear dependency, spanning, and basis|span]], $\vec{x}_1,\vec{y}_1$ can be rewritten as:
$$
\vec{x}=\sum c_i\vec{a_i} \space\space\text{ and } \space\space \vec{y}=\sum p_k\vec{b}_k
$$
...
In a similar fashion, we can simply take a $c \in \mathbb{R}$ and multiply it with, lets say $\vec{x} \in \mathcal{A}$, to obtain
$$
c \vec{x} = \sum cc_i\vec{i}
$$
Which once again, is a valid linear combination of vectors within $\mathcal{A}$.

As $\{span \text{ } \mathcal{A}\}$ contains the zero vector and satisfies closure under vector addition and multiplication (and is a subset of vectors of $\mathcal{A}$), it is a valid subspace of $\mathcal{A}$. via the [[2.1 Subspaces#Theorem 2.1 Criteria for a subspace|subspace criterion]]
## 2.6 Matrix
...
Prove that the row space and column space of every matrix are valid [[2.1 Subspaces#2.1 Subspaces|subspaces]]
...
Row and column spaces are spans therefore they are subspaces as previously established. [[#Example 2.5]]
