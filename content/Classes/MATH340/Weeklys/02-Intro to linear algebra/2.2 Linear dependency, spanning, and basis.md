...
let $S = \{\vec{v}_1,\vec{v}_2,...\vec{v}_m\}$ be a subset of the [[1.4 Vector spaces|vector space]] $\mathbb{R}^n$, and $\vec{w}$ be a vector in $\mathbb{R}^n$. We say $w$ is a linear combination of elements of $S$ if $\vec{w}=c_1\vec{v}_1+...+c_m\vec{v}_m$ for some $c_1,...c_m \in \mathbb{R}$. By definition, if $S$ is the empty set, then the only linear combination of elements of $S$ is $\vec{0}$. 

![relevant 3blue1brown vid](https://www.youtube.com/watch?v=k7RM-ot2NWY&ab_channel=3Blue1Brown)
