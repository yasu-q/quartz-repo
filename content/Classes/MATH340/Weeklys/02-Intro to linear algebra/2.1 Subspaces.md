...
## 2.1 Subspaces
A subspace $W$ of $\mathbb{R}^n$ is a subspace of $\mathbb{R}^n$ if $W$ along with the same operations of $\mathbb{R}^n$ satisfy all properties of a [[1.4 Vector spaces|vector space]].
...
We will use the subspace criterion ([[#Theorem 2.1 Criteria for a subspace]]). Note that $\vec{0}$ belongs to both $W \text{ and } U$ and thus must be within $W \cap U$

Suppose $\vec{x},\vec{y} \in U \cap W$ and $c \in \mathbb{R}$. By the definition of an intersection, $\vec{x}$ and $\vec{y}$ are both in $U$ and $W$. Since $U$ and $W$ are both subspace, by Theorem 2.1 we have the following: 
$$
\vec{x} + \vec{y} \in W, \vec{x} + \vec{y}  \in U, c\vec{x} \in W, c\vec{y} \in U
$$
Thus, by the definition of an intersection, $\vec{x} + \vec{y} \in U \cap W$, and $c\vec{x} \in U \cap W$. Which by the definition of a subspace, demonstrates how $W \cap U$ is a valid subspace on $\mathbb{R}^n$.
***
