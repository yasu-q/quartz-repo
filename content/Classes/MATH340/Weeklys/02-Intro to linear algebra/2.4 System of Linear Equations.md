...
\end{matrix}
$$
In order to solve this system, we will utilize elimination. In order to solve this in such a fashion, we will utilize an augmented [[2.3 Example subspaces, row, and col spaces#2.6 Matrix|matrix]] which looks something like the following for the system we are solving 
$$
\left( 
    \begin{array}{ccc|c}
        3 & 2 & -1 & 4 \\
...
## Theorem 2.4
Every matrix can be turned into a matrix in reduced echelon form by applying the three elementary row operations. Furthermore, the reduced echelon form for any matrix (if it has one) is **unique**
### 2.7 Pivots
...
## Theorem 2.5 
Any homogenous system that has less equations than variables has a nontrivial solution. Infinite, in fact.
### Corollary
