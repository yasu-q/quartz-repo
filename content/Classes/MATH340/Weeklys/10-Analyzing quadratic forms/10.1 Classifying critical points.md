...
## 10.5 Hessian matrix
Let $U$ be an open subset of $\mathbb{R}^n$. Suppose $f : U → \mathbb{R}$ is a $C^2$ function. The Hessian matrix of $f$ at a point ${\bf a} ∈ U$ is the $n × n$ matrix whose $(i, j)$ entry is $D_iD_j f ({\bf a})$. The Hessian matrix of $f$ at a is denoted by $H_f ({\bf a})$
## Theorem 10.8
